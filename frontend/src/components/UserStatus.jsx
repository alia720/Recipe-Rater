    ...
    return (
      <div className={`relative ${isMobile ? "" : "ml-3"}`}>
        ...
        <button
          ref={buttonRef}
          type="button"
          className={`flex max-w-xs items-center rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 transition-transform duration-150 ease-in-out hover:scale-105 ${
            isMobile ? "mx-auto" : ""
          }`} // Center button if in mobile menu
          id="user-menu-button"
        ...
        >
          ...
          {!isMobile && user?.name && (
            <>
              <span className="text-white font-medium ml-2 mr-1">
                {user.name}
              </span>
              <ChevronDownIcon
        ...
        {showDropdown && (
          ...
          >
            ...
            <button
              ...
              id="user-menu-item-1"
            >
              <ArrowRightOnRectangleIcon className="inline-block h-4 w-4 mr-2 align-text-bottom" />{" "}
              {/* Icon */}
              Sign out
            </button>
          </div>
    ...
    return (
      <div
        className={`flex items-center ${
          isMobile ? "flex-col gap-4 w-full px-4" : "gap-3"
        }`}
      >
        {/* Secondary Button Style */}
        <Link
          to="/login"
          className={`px-4 py-1.5 rounded-md text-sm font-medium text-gray-200 border border-gray-600 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:ring-offset-gray-900 transition-colors duration-150 ${
        ...
        <Link
          to="/signup"
          className={`px-4 py-1.5 rounded-md text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900 transition-colors duration-150 ${
            isMobile ? "w-full text-center" : ""
          }`} // Full width on mobile
        >
          Sign Up
