...
const EditRecipe = () => {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (res.status === 404) throw new Error('Recipe not found (maybe deleted?).');
                throw new Error(errorData.error || `Failed to update recipe (Status: ${res.status})`);
            }
    ...
    if (loading && !originalRecipe) {
        return <div className="flex items-center justify-center min-h-screen bg-black text-white">Loading recipe details...</div>;
    }
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4">
                <div className="bg-red-900 border border-red-700 text-red-100 px-4 py-3 rounded text-center">
                    <p>{fetchError}</p>
                    <Link to="/profile" className="text-blue-300 hover:text-blue-200 mt-2 block">&larr; Back to Profile</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-black text-white p-4">
            <form onSubmit={handleUpdate} className="bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-2xl">
                <h2 className="text-2xl font-bold mb-6 text-center">Edit Recipe</h2>

                {error && <p className="text-red-500 mb-4 text-center bg-red-900 p-2 rounded border border-red-700">{error}</p>}

                <div className="space-y-4">
                    {/* --- Name Input (remains the same) --- */}
                    <div>
                        <label htmlFor="name" className="block text-gray-300 mb-2">Recipe Name</label>
                        <input
                            id="name" type="text" value={name} onChange={(e) => setName(e.target.value)}
                            className="w-full p-2 rounded bg-gray-800 text-white border border-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" required
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full p-2 rounded bg-gray-800 text-white border border-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" rows="3"
                        />
                    </div>

                    {/* --- Ingredients Textarea --- */}
                    <div>
                        <label htmlFor="ingredients" className="block text-gray-300 mb-2">Ingredients (one per line recommended)</label>
                        <textarea
                            id="ingredients" value={ingredients} onChange={(e) => setIngredients(e.target.value)}
                            className="w-full p-2 rounded bg-gray-800 text-white border border-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" rows="5" required
                        />
                    </div>

                    {/* --- Instructions Textarea --- */}
                    <div>
                        <label htmlFor="instructions" className="block text-gray-300 mb-2">Instructions</label>
                        <textarea
                            id="instructions" value={instructions} onChange={(e) => setInstructions(e.target.value)}
                            className="w-full p-2 rounded bg-gray-800 text-white border border-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" rows="8" required
                        />
                    </div>


                    <div className="flex justify-between items-center pt-4">
                        <Link to="/profile" className="text-gray-400 hover:text-gray-200 transition-colors duration-150">Cancel</Link>
                        <button
