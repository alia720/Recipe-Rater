...
const Profile = () => {
    ...
    const fetchUserRecipes = async (userId) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.message || `Failed to fetch recipes (Status: ${res.status})`);
            }
    ...
    return (
        <div className="p-4 md:p-6 bg-black min-h-screen text-white relative" /* Added relative for absolute positioning context */>
            ...
            <h1 className="text-3xl md:text-4xl font-bold mb-6 text-center md:text-left">Profile</h1>

            {/* User Info Section */}
            <div className="bg-gray-900 p-5 md:p-6 rounded-lg shadow-lg w-full max-w-4xl mx-auto mb-8">
                <h2 className="text-xl md:text-2xl font-semibold text-white mb-4">User Information</h2>
                {user ? (
                    <div className="space-y-2 text-gray-300">
                        <p><span className="font-medium text-gray-100">Name:</span> {user.name}</p>
                        <p><span className="font-medium text-gray-100">Username:</span> {user.username}</p>
                    </div>
                ) : (
            ...
            <div className="bg-gray-900 p-5 md:p-6 rounded-lg shadow-lg w-full max-w-4xl mx-auto">
                <h2 className="text-xl md:text-2xl font-semibold text-white mb-4">My Recipes</h2>

                {/* Loading Indicator */}
                {loading && (
                    <div className="flex items-center justify-center space-x-3 text-gray-300 py-8">
                        {/* SVG Spinner */}
                        <svg className="animate-spin h-6 w-6 text-blue-400" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {/* Error Message */}
                {error && !loading && (
                    <div className="bg-red-900 border border-red-700 text-red-100 px-4 py-3 rounded relative text-center" role="alert">
                ...
                {!loading && !error && recipes.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                        {recipes.map((recipe) => (
                            ...
                            >
                                <h3 className="text-white text-lg font-bold mb-3 truncate pointer-events-none" title={recipe.name} /* Added pointer-events-none */ >
                                    {recipe.name}
                                </h3>

                                {recipe.main_photo && (
                                    <div className="mb-3 aspect-video overflow-hidden rounded pointer-events-none" /* Added pointer-events-none */ >
                                        <img
                                ...
                                <div className="space-y-1 max-h-40 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800 pr-2 pointer-events-none" /* Added pointer-events-none */ >
