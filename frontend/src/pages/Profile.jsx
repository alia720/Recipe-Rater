...
const Profile = () => {
  ...
  useEffect(() => {
    if (user?.user_id) fetchUserRecipes(user.user_id);
  }, [user]);
      ...
      <div className="p-4 bg-black min-h-screen text-white">
        ...
        <div className="bg-gray-900 p-6 rounded-lg max-w-4xl mx-auto">
          ...
          {loading ? (
              ...
              <div className="bg-red-900 p-4 rounded text-red-100">Oops! {error}</div>
          ) : recipes.length === 0 ? (
              <p className="italic text-gray-400 text-center py-8">You haven't added any recipes yet.</p>
          ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {recipes.map(r => (
                    <div key={r.recipe_id} className="relative bg-gray-800 p-4 rounded-lg hover:shadow-lg">
                      ...
                      {r.main_photo && <img src={r.main_photo} alt={r.name} className="mb-2 w-full h-32 object-cover rounded" />}
